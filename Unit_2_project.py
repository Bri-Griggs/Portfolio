...
class Car:
  def __init__(self, make: str, model: str, year: int, cost_price: int, selling_price: int, color: str):
    self.make = make
    self.model = model
    self.year = year
...
def pullnames() -> list[Account]: 
  ...
  return [Account(i[0], bytes(i[1], 'utf-8'), bytes(i[2], 'utf-8'), i[3] == 'True', i[4] == "True") for i in userlist]
...
def searchUser(userlist: list[Account]) -> None:
  searchfor = input("Enter the name of the account you want to search for: ").lower().strip()
  print()
  for user in userlist:
    if searchfor in user.username:
      print(f'{user.username}: {Fore.BLUE}{"Customer" if not user.is_moderator and not user.is_admin else "Admin" if user.is_admin else "Moderator"}{Fore.RESET}')
...
def AddCar() -> Optional[Car]:
  while True:
    ...
    return Car("Solksvagen", inputModel, inputYear, inputCost, inputSell, inputColor)
...
def RemoveCar(car_list) -> Optional[Car]:
  ...
  while cartodelete != 'back':
    ...
    for index, car in enumerate(car_list):
      print(f'{index + 1}. {car.year} {car.model} - {Fore.BLUE}{price_format(car.selling_price)}{Fore.RESET}')
  ...
  while running:
    ...
    if state == 'start':
      ...
      while current_user is None and running:
        print("Welcome! Would you like to [Log in], [Create] an account, or [Quit].")
    ...
    ###==={Customer View}===###
      ...
      print(f'Hello {Fore.BLUE}{current_user.username}{Fore.RESET}, Welcome to the Solkvwagen™ Sales Management System, what would you like to do?')
      choice = input(f"Browse vehicles by [Year], [Price], [Color], {'[Manage], ' if current_user.is_admin or current_user.is_moderator else ''}or [Logout]?\n> ").strip().lower()
      print()
      ...
      elif choice == 'manage' and (current_user.is_moderator or current_user.is_admin):
        state = 'manage'
        os.system('clear')
    ...
    ###==={Manager  View}===###
      ...
      if choice == 'users':
        ...
        while choice != 'back':
          print(f'Current total users: {Fore.BLUE}{len(AllCurrentUsers)}{Fore.RESET}')
          print(f"Would you like to [Search] for a user, [Display] all users, {'[Promote] a user, [Demote] a user, ' if current_user.is_admin else ''}[Delete] a user, or [Back].")
          ...
          ###==={Display All Users}===###
            ...
            sorted_users = sorted(AllCurrentUsers, key=lambda x: not x.is_moderator)
          ...
          ###==={Promote Users (Admin)}===###
            ...
            promotables = [user for user in AllCurrentUsers if not user.is_moderator and not user.is_admin]
          ...
          ###==={Demote Users (Admin)}===###
            ...
            demotables = [user for user in AllCurrentUsers if user.is_moderator and not user.is_admin]
            if len(demotables) != 0:
          ...
          ###===={Delete Users}===###
            ...
            nonmod = [i for i in AllCurrentUsers if not i.is_moderator and not i.is_admin]
            ...
            if len(nonadmin) != 0 and current_user.is_admin: # type: ignore
              ...
              for user in AllCurrentUsers:
                if not user.is_admin and not user.is_moderator and user.username == WhoToDelete:
                  AllCurrentUsers.remove(user)
            else:
          ...
          ###==={Back to Manage}===###
            ...
            with open("userlog.txt", 'w') as f:
              for user in AllCurrentUsers:
                f.write(f"{user.username}, {user.password}, {user.salt}, {user.is_moderator}, {user.is_admin}\n")
            os.system('clear')
      ...
      ###==={Manage Stock}===###
        ...
        while choice != 'back':
          print(f"Would you like to [View] Cars, [Add] New Cars, [Remove] Cars, or Go [Back]")
          choice = input("> ").lower().strip()
          print()
          ...
          ###==={Back to Manage}===###
            ...
            with open("carlog.txt", 'w') as f:
              for car in car_list:
               f.write(f'{car.make}, {car.model}, {car.year}, {car.cost_price}, {car.selling_price}, {car.color}\n')
            os.system('clear')
